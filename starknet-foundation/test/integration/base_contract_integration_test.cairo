...
mod tests {
    ...
    fn test_complete_workflow() {
        ...
        assert(BaseContract::is_user_allowed(@state, user1), 'User1 should still be allowed');
        assert(!BaseContract::is_user_allowed(@state, user2), 'User2 should be removed');
        assert(!BaseContract::is_user_allowed(@state, user3), 'User3 should be removed');
    }
    ...
    fn test_edge_cases() {
        ...
        let result = BaseContract::update_api_key(ref state, 789_felt252);
        ...
        let result = BaseContract::update_user_limit(ref state, 2_u32);
        ...
        BaseContract::batch_update_users(ref state, empty_add, empty_remove);
    ...
    fn test_bitmap_efficiency() {
        ...
        set_caller_address(owner);
        let mut users_to_remove = ArrayTrait::new();
        BaseContract::batch_update_users(ref state, users, users_to_remove);

        assert(state.user_count.read() == 512, 'All users should be added');
        
        // Verify random users are in bitmap
        assert(BaseContract::is_user_allowed(@state, ContractAddress::from(0_u32)), 'User 0 should be allowed');
        assert(BaseContract::is_user_allowed(@state, ContractAddress::from(255_u32)), 'User 255 should be allowed');
        assert(BaseContract::is_user_allowed(@state, ContractAddress::from(511_u32)), 'User 511 should be allowed');
    }
} 